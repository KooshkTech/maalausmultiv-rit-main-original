...
const processSteps = [
  {
    icon: ClipboardList,
    title: 'Arvio ja tarjous',
    desc: 'Käymme paikan päällä ja laadimme ilmaisen, sitouttamattoman tarjouksen.',
  },
  {
    icon: Sparkles,
    title: 'Esikäsittely',
    desc: 'Puhdistamme pinnat, poistamme irtoavan maalin ja korjaamme halkeamat.',
  },
  {
    icon: Paintbrush,
    title: 'Toteutus',
    desc: 'Pohjustamme ja maalaamme ammattimaisesti, siististi ja aikataulussa.',
  },
  {
    icon: BadgeCheck,
    title: 'Takuu ja lopetus',
    desc: 'Tarkistamme lopputuloksen ja annamme kirjallisen takuun jopa 5 v.',
  },
];

const serviceFaqs = [
  {
    q: 'Kuinka pian pääsette aloittamaan?',
    a: 'Aikataulu riippuu sesongista ja työn laajuudesta. Parhaimmillaan aloitamme muutaman päivän kuluessa tarjouksen hyväksymisestä.',
  },
  {
    q: 'Annaatteko takuun työjäljestä?',
    a: 'Kyllä. Annamme kirjallisen takuun maalaustyöjäljestä jopa 5 vuotta. Takuu kattaa maalipinnan kestävyyden normaaleissa sääolosuhteissa.',
  },
  {
    q: 'Maksatteko arviokäynnin?',
    a: 'Ei, arviokäynti on aina ilmainen ja sitoutumaton. Et sitoudu mihinkään arviokäynnin perusteella.',
  },
  {
    q: 'Voitteko työskennellä asutussa asunnossa?',
    a: 'Kyllä. Suojamme kalusteet ja lattiat huolellisesti ja käytämme matalahajuisia maaleja. Voimme maalata huone kerrallaan.',
  },
];
...
export function ServiceDetailPage() {
  ...
  if (!service) {
    return <Navigate to="/palvelut" replace />;
  }
  ...
  const catIndex = categoryServices.findIndex(
    (s) => s.slug === service.slug,
  );

  const next =
    categoryServices[(catIndex + 1) % categoryServices.length];

  const serviceImage =
    images.services[
      service.slug as keyof typeof images.services
    ] || service.image;
        ...
        <div className="container-base grid gap-12 lg:grid-cols-12">
          <Reveal className="lg:col-span-7">
            ...
            <p className="mt-4 text-navy-600">
              Jokainen {service.title.toLowerCase()}-työmme
              sisältää seuraavat vaiheet. Näin taatumme, että
              lopputulos kestää sään ja ajan.
            </p>
            ...
            <ul className="mt-6 flex flex-col gap-3">
              {service.bullets.map((b) => (
                <li
                  key={b}
                  className="flex items-start gap-3 rounded-xl border border-navy-100 bg-navy-50/50 p-4"
                >
          ...
          >
            <div className="card sticky top-24 p-8">
              ...
              <p className="mt-3 text-sm text-navy-600">
                Tarjous on ilmainen ja sitoutumaton. Käymme
                paikan päällä tutkimassa kohteen ja laadimme
                tarkan aikataulun ja hinta-arvion.
              </p>
              ...
              <div className="mt-6 flex flex-col gap-3">
                ...
                <Link
                  to="/kustannuslaskuri"
                  className="text-center text-sm font-semibold text-orange-600 transition hover:text-orange-700"
                >
        ...
        <div className="container-base max-w-3xl">
          ...
          <div className="mt-10 space-y-3">
            {serviceFaqs.map((f, i) => (
              <details
                key={i}
                className="group rounded-xl border border-navy-100 p-5"
              >
                <summary className="flex cursor-pointer items-center justify-between text-sm font-semibold text-navy-800">
                  {f.q}

                  <span className="text-xl text-navy-400 transition group-open:rotate-45">
                    +
                  </span>
                </summary>
                ...
                >
                  <div className="relative aspect-[16/10] overflow-hidden">
                    <img
                      src={
                        images.services[
                          s.slug as keyof typeof images.services
                        ] || s.image
                      }
                      alt={s.title}
