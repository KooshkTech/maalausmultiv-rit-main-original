...
import { localServicePath, hasPriorityLocalPage } from '@/data/localSeo';
...
const processSteps = [
  ...
  { icon: BadgeCheck, title: 'Takuu ja lopetus', desc: 'Tarkistamme lopputuloksen ja annamme kirjallisen takuun 2 vuotta.' },
];
...
const serviceFaqs = [
  ...
  { q: 'Annaatteko takuun työjäljestä?', a: 'Kyllä. Annamme kirjallisen takuun maalaustyöjäljestä 2 vuotta. Takuu kattaa maalipinnan kestävyyden normaaleissa sääolosuhteissa.' },
        ...
        <div className="container-base grid gap-12 py-16 lg:grid-cols-12 lg:items-center lg:py-24">
          <Reveal className="lg:col-span-6">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/yhteystiedot" onClick={() => trackCtaClick('Pyydä ilmainen tarjous', 'service_page_hero')} className="btn-primary">
                Pyydä ilmainen tarjous
                <ArrowRight className="h-4 w-4" />
              </Link>
              <a href={company.phoneHref} onClick={() => trackPhoneClick('service_page_hero')} className="btn-ghost-light">
          ...
          <Reveal delay={150} className="lg:col-span-5">
            <div className="card sticky top-24 p-8">
              ...
              <div className="mt-6 flex flex-col gap-3">
                <Link to="/yhteystiedot" onClick={() => trackCtaClick('Täytä tarjouspyyntö', 'service_page_sidebar')} className="btn-primary w-full">
                  Täytä tarjouspyyntö
                  <ArrowRight className="h-4 w-4" />
                </Link>
                <a href={company.phoneHref} onClick={() => trackPhoneClick('service_page_sidebar')} className="btn-outline w-full">
                  <Phone className="h-4 w-4" />
                  {company.phone}
                </a>
      ...
      <section className="section-pad bg-white">
        <div className="container-base">
          <div className="text-center">
            ...
            <h2 className="mt-4 font-display text-3xl font-bold text-navy-900">
              {service.title} lähialueillamme
            </h2>
            <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-navy-600">
              Tarkista palvelualueesi ja pyydä tarjous. Laajennamme palvelua kohteen mukaan myös Uudenmaan ulkopuolelle.
            </p>
          </div>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {priorityCities.map((city) => (
              <Link
                key={city.slug}
                to={hasPriorityLocalPage(service.slug, city.slug) ? localServicePath(service.slug, city.slug) : `/palvelualueet/${city.slug}`}
                className="card group p-5 transition hover:-translate-y-0.5 hover:shadow-lift"
              >
                <p className="font-display text-lg font-bold text-navy-900">{service.title} {city.locative}</p>
                <span className="mt-2 inline-flex items-center gap-1.5 text-sm font-semibold text-orange-600">
                  {hasPriorityLocalPage(service.slug, city.slug) ? 'Katso paikallinen palvelu' : 'Katso palvelualue'} <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-1" />
                </span>
              </Link>
            ))}
          </div>
