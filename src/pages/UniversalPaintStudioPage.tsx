...
export function UniversalPaintStudioPage() {
  ...
  const [message, setMessage] = useState('Maalaa vapaasti missä tahansa kuvassa.');
  ...
  const loadFile = (file?: File) => {
    ...
    setMessage('Valitse väri ja maalaa. Älykäs täyttö toimii ilman ulkoista AI-kiintiötä.');
  };
  ...
  const smartFill = (seed: Point) => {
    ...
    const sx = Math.floor(seed.x), sy = Math.floor(seed.y); const si = (sy*w+sx)*4;
    const sr=data[si], sg=data[si+1], sb=data[si+2];
    const seen = new Uint8Array(w*h); const mask = new Uint8Array(w*h); const queue = new Int32Array(w*h); let head=0,tail=0;
    const start=sy*w+sx; queue[tail++]=start; seen[start]=1; let count=0; const maxPixels=Math.min(w*h, 650000);
    const match=(idx:number)=>{ const i=idx*4; const dr=data[i]-sr, dg=data[i+1]-sg, db=data[i+2]-sb; return Math.sqrt(dr*dr+dg*dg+db*db) <= tolerance; };
    while(head<tail && count<maxPixels){ const idx=queue[head++]; if(!match(idx)) continue; mask[idx]=1; count++; const x=idx%w, y=Math.floor(idx/w);
    ...
    const overlay = targetCtx.createImageData(w,h); const rgb = parseInt(color.slice(1),16); const r=(rgb>>16)&255,g=(rgb>>8)&255,b=rgb&255;
    for(let i=0;i<mask.length;i++){ if(mask[i]){ const p=i*4; overlay.data[p]=r; overlay.data[p+1]=g; overlay.data[p+2]=b; overlay.data[p+3]=Math.round(255*opacity); } }
    targetCtx.putImageData(overlay,0,0); commit();
    setMessage(count >= maxPixels ? 'Täyttö pysäytettiin turvallisuusrajalle. Pienennä toleranssia ja kokeile uudelleen.' : 'Älykäs täyttö valmis. Jos alue meni yli reunan, kumoa ja pienennä toleranssia.');
  };

  const pointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (!imageUrl) return; event.currentTarget.setPointerCapture(event.pointerId);
    const p = pointFromEvent(event); if (!p) return;
    if (tool === 'smart') { smartFill(p); return; }
    drawingRef.current=true; lastRef.current=p; drawLine(p,p);
  };
  const pointerMove = (event: ReactPointerEvent<HTMLDivElement>) => { if(!drawingRef.current || !lastRef.current) return; const p=pointFromEvent(event); if(!p) return; drawLine(lastRef.current,p); lastRef.current=p; };
  const pointerUp = () => { if(drawingRef.current){ drawingRef.current=false; lastRef.current=null; commit(); } };
    ...
    <div className="container-base max-w-6xl">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_310px]">
        ...
        <aside className="space-y-4">
          <div className="card p-4"><h2 className="font-bold text-navy-950">1. Työkalu</h2><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={()=>setTool('brush')} className={tool==='brush'?'btn-primary':'btn-outline'}><Paintbrush className="size-4"/>Sivellin</button><button onClick={()=>setTool('roller')} className={tool==='roller'?'btn-primary':'btn-outline'}><PaintRoller className="size-4"/>Tela</button><button onClick={()=>setTool('smart')} className={tool==='smart'?'btn-primary':'btn-outline'}><Sparkles className="size-4"/>Älykäs täyttö</button><button onClick={()=>setTool('eraser')} className={tool==='eraser'?'btn-primary':'btn-outline'}><Eraser className="size-4"/>Pyyhin</button></div><p className="mt-3 text-xs leading-5 text-navy-600">Älykäs täyttö: napauta haluamaasi pintaa. Selain etsii samanvärisen yhtenäisen alueen. Tämä ei käytä Hugging Face -kiintiötä.</p></div>
