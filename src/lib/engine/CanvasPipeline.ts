...
// Coats describe progressive paint build-up. The coverage slider remains the final authority.
const COAT_BUILD: Record<1 | 2 | 3 | 4, number> = { 1: 0.72, 2: 0.90, 3: 0.98, 4: 1 };
...
export class CanvasPipeline implements IRenderPipeline {
  ...
  applyPaintMask(mask: MaskLayer, params: PaintParams): void {
    ...
    for (let y = 0; y < source.height; y += 1) {
      ...
      for (let x = 0; x < source.width; x += 1) {
        ...
        const targetR = clamp(tr * textureShade), targetG = clamp(tg * textureShade), targetB = clamp(tb * textureShade);
        let alpha = clamp01(maskAlpha * coverage * coatBuild);
        if (coverage >= 0.95 && params.coat >= 3 && maskAlpha >= 0.95) alpha = Math.max(alpha, params.coat === 4 ? 1 : 0.98);
  ...
  applyCleaningMask(mask: MaskLayer, params: CleaningParams): void {
    const source = this.requireBase(); this.assertMask(mask);
    const output = new ImageData(new Uint8ClampedArray(source.data), source.width, source.height);
    const data = output.data, intensity = clamp01(params.intensity), qualityScale = params.quality === 'final' ? 1 : 0.82;
    for (let y = 0; y < source.height; y += 1) for (let x = 0; x < source.width; x += 1) {
      const idx = y * source.width + x, maskAlpha = mask.alpha[idx] / 255; if (maskAlpha <= 0) continue;
      const p = idx * 4, local = intensity * maskAlpha * qualityScale;
      for (let c = 0; c < 3; c += 1) { const orig = source.data[p + c]; const cleaned = clamp(orig + (128 - orig) * (0.25 * local) + 15 * local); data[p + c] = clamp(orig * (1 - maskAlpha) + cleaned * maskAlpha); }
    }
    this.ctx.putImageData(output, 0, 0);
  }
