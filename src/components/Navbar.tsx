...
import { Menu, X, Phone, PaintRoller, ChevronDown, Brush, ArrowRight, Palette } from 'lucide-react';
    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 shadow-soft backdrop-blur-md' : 'bg-white/90 backdrop-blur-sm'}`}>
      <nav className="container-base flex h-16 items-center justify-between lg:h-20">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <Link to="/varikamu" className="inline-flex items-center gap-1.5 rounded-full border border-orange-200 bg-orange-50 px-3.5 py-2 text-xs font-bold text-orange-700 transition hover:bg-orange-100"><Palette className="h-4 w-4" />Suunnittele & laske</Link>
      ...
      {open && (
        <div className="lg:hidden border-t border-navy-100 bg-white shadow-lift">
          <div className="container-base max-h-[calc(100vh-4rem)] overflow-y-auto px-5 py-4">
            <Link to="/varikamu" className="mb-3 flex items-center justify-center gap-2 rounded-xl bg-orange-50 py-3 text-sm font-extrabold text-orange-700"><Palette className="h-5 w-5" />Kokeile värejä ja laske hinta</Link>
