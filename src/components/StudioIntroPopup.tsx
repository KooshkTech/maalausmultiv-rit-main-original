...
export function StudioIntroPopup() {
  ...
  return (
    <aside className="fixed inset-x-4 bottom-4 z-50 mx-auto max-w-xl rounded-3xl border border-navy-100 bg-white p-5 shadow-2xl sm:p-6" role="dialog" aria-label="Tutustu maalaustyökaluihin">
      ...
      <p className="mt-2 max-w-lg text-sm leading-6 text-navy-600">VäriKamu auttaa kokeilemaan maalausideoita omassa kuvassa. Maalauslaskurilla arvioit työn laajuutta ja saat suuntaa-antavan hintahaarukan.</p>
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <Link to="/maalauslaskuri" onClick={dismiss} className="flex min-h-20 items-center gap-3 rounded-2xl bg-orange-50 p-4 text-navy-950 hover:bg-orange-100">
          <Sparkles className="size-6 text-orange-600" />
          <span><strong className="block">Maalauslaskuri</strong><span className="text-xs text-navy-600">Alustava hintahaarukka</span></span>
        </Link>
      </div>
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs font-semibold text-navy-500">
        ...
        <Link to="/palvelut" onClick={dismiss} className="hover:text-orange-600">Tutustu maalauspalveluihin</Link>
      </div>
    </aside>
  );
}
